  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      ...
      if (!token) {
        ...
        router.push("/login/AgentLogin");  // Router works now
        return;
      }

      try {
        const response = await fetch("https://avetiumbackupservice.avetiumconsult.com/api/auth/agent/", {
          headers: {
    ...
    <DashboardLayout>
      <div className="flex flex-col lg:flex-row lg:items-center gap-3 lg:gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3 mt-2 lg:mt-0 lg:ml-auto text-sm sm:text-[15px] font-medium">
          <h2 className="text-gray-600">{agentData.location}</h2>
          <Image src="/icons/dot.png" alt="Dot" width={8} height={8} />
          <p className="text-gray-600">{agentData.connectionStatus}</p>
        </div>
      </div>
